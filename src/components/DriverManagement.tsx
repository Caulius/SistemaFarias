  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.name.trim()) {
      try {
        ...
        alert('Erro ao adicionar motorista. Tente novamente.');
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Gerenciar Motoristas</h2>
        <button
      ...
      {showForm && (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
          <h3 className="text-lg font-semibold mb-4 text-gray-900 dark:text-white">Novo Motorista</h3>
      ...
      {drivers.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-sm text-gray-400 dark:text-gray-500">Adicione motoristas para começar a criar programações</p>
        </div>
      )}
    </div>
  );
